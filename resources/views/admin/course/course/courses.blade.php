...
@section('content')
    <h1 class="template-title">Khoá học</h1>
    <ul class="post-status">
        <li @if(Request::route()->status === null) class="active" @endif>
            <a href="{{route('GET_COURSE_ROUTE')}}">Tất cả</a> <span>({{$count['all']}})</span>
        </li>
        @if($count['publish'] !== 0)
            <li @if(Request::route()->status === 'publish') class="active" @endif>
                <a href="{{route('GET_COURSE_ROUTE', 'publish')}}">Đã xuất bản</a> <span>({{$count['publish']}})</span>
            </li>
        @endif
        @if($count['draft'] !== 0)
            <li @if(Request::route()->status === 'draft') class="active" @endif>
                <a href="{{route('GET_COURSE_ROUTE', 'draft')}}">Bản nháp</a> <span>({{$count['draft']}})</span>
            </li>
        @endif
        @if($count['pending'] !== 0)
            <li @if(Request::route()->status === 'pending') class="active" @endif>
                <a href="{{route('GET_COURSE_ROUTE', 'pending')}}">Chờ duyệt</a> <span>({{$count['pending']}})</span>
            </li>
        @endif
        @if($count['trash'] !== 0)
            <li @if(Request::route()->status === 'trash') class="active" @endif>
                <a href="{{route('GET_COURSE_ROUTE', 'trash')}}">Thùng rác</a> <span>({{$count['trash']}})</span>
            </li>
        @endif
    </ul>
            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    <div class="kt-portlet__head-actions">
                        <a href="{{route('GET_CREATE_COURSE_ROUTE')}}" class="btn btn-brand btn-elevate btn-icon-sm">
