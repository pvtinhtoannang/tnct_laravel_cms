;(function ($, window, document, undefined) {
    ...
    /** Detect CSS pointer-events property events are normally disabled on the dragging element to avoid conflicts https://github.com/ausi/Feature-detection-technique-for-pointer-events/blob/master/modernizr-pointerevents.js */ var hasPointerEvents = (function () {
    ...
    Plugin.prototype = {
        init: function () {
            ...
            this.dragEl.css({'left': e.pageX - mouse.offsetX, 'top': e.pageY - mouse.offsetY});/* total depth of dragging item*/
            ...
            this.dragEl.css({'left': e.pageX - mouse.offsetX, 'top': e.pageY - mouse.offsetY});/* mouse position last events*/
            ...
            mouse.dirAx = newAx;/** move horizontal */
            if (mouse.dirAx && mouse.distAxX >= opt.threshold) {/* reset move distance on x-axis for new phase*/
                mouse.distAxX = 0;
                prev = this.placeEl.prev(opt.itemNodeName);/* increase horizontal level if previous sibling exists and is not collapsed*/
                if (mouse.distX > 0 && prev.length && !prev.hasClass(opt.collapsedClass)) {/* cannot increase level when item above is collapsed*/
                    list = prev.find(opt.listNodeName).last();/* check if depth limit has reached*/
                    depth = this.placeEl.parents(opt.listNodeName).length;
                    if (depth + this.dragDepth <= opt.maxDepth) {/* create new sub-level if one doesn't exist*/
                        if (!list.length) {
                            list = $('<' + opt.listNodeName + '/>').addClass(opt.listClass);
                            list.append(this.placeEl);
                            prev.append(list);
                            this.setParent(prev);
                        } else {/* else append to next level up*/
                            list = prev.children(opt.listNodeName).last();
                            list.append(this.placeEl);
                        }
                    }
                }/* decrease horizontal level*/
                if (mouse.distX < 0) {/* we can't decrease a level if an item preceeds the current one*/
            ...
            var pointElRoot = this.pointEl.closest('.' + opt.rootClass),
                isNewRoot = this.dragRootEl.data('nestable-id') !== pointElRoot.data('nestable-id');/** move vertical */
            if (!mouse.dirAx || isNewRoot || isEmpty) {/* check if groups match if dragging over new root*/
